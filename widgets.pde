abstract class Component extends Group {
  int x,y,w,h;
  String _id = "";
  int colorespecifico = 220;
  int colorletra = #000000;
  ...
  Component colorespecifico(int col) {
    colorespecifico = col; return this;
  }
  
  Component colorletra(int col) {
    colorletra = col; return this;
  }
...
class Button extends Control {
  ...
  void update() {
    ...
    add(newRect(x,y,w,h,h/30).fillColor(colorespecifico));
    add(newText(str,x+w/2-str.length()*textAdjust,y+h/2).fontSize(25).fontColor(colorletra));
  }
...
class TextComponent extends Control {
  ...
  void update() {
    ...
    add(newRect(x,y,w,h).fillColor(colorespecifico));
    add(newText(label,x+4,y+h/2).fontSize(55).fontColor(colorletra));
  }
}
